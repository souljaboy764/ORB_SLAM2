...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FramePublisher *pFramePublisher, MapPublisher *pMapPublisher, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpPublisher(NULL),
    mpFramePublisher(pFramePublisher), mpMapPublisher(pMapPublisher), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    mMinOdomDist = fSettings["MinOdomDist"];
...
void Tracking::SetPublisher(Publisher *pPublisher)
{
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        /*if(InitOdom)
        {
            ...
            float dist = sqrt((InitOdom->pose.pose.position.x - CurrOdom->pose.pose.position.x)*(InitOdom->pose.pose.position.x - CurrOdom->pose.pose.position.x)
...
void Tracking::Reset()
{
    ...
    if(mpPublisher)
    {
        mpPublisher->RequestStop();
        while(!mpPublisher->isStopped())
            usleep(3000);
    }
